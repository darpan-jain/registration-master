...
def register():
	...
	inp=Tk()
	info=Label(inp, height=5, width=30,text="Enter your serial number",font=("Helvetica bold", 20))
	textBox=Text(inp, height=1, width=5,font=("Helvetica bold", 50))
	info.pack()
	textBox.pack()
	buttonCommit=tk.Button(inp, height=6, width=22, text="Submit",command=lambda:retrieve_input(textBox,inp),activebackground="black", activeforeground="cyan", bd=4, bg="#123d63", fg="gold", font=("Helvetica bold", 15))
	...
	full_name = first_name[int(member)] + " " + last_name[int(member)]
	
	while (click):
		f = open('./utils/event_reg.txt','r');
		data_set = json.loads(f.read());
		t=open('./utils/event_time.txt','r');
		...
		while True:
			...
			if (recval>=7):
				...
				t = open('./utils/event_time.txt', 'w');
				t.write(json.dumps(time_data))
				f = open('./utils/event_reg.txt', 'w');
				...
				while(True):
					...
					cv2.putText(img,"Welcome :)",(50,300), font, 3,(47, 160, 181),6,cv2.LINE_AA)
	...
	while(True):
		...
		for (i, rect) in enumerate(rects):
			...
			if(len(aligns) == 1 and face_pos=='Center') :
				...
				cv2.putText(frame,per,(rect[0],rect[1]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),3,cv2.LINE_AA)
				# cv2.putText(frame,str(recog_data[0][1])+"%",(rect[0],rect[1]),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),1,cv2.LINE_AA)
				# detect=detect+1
				# if(detect>10):
				# 	det_peeps=det_peeps+1
				# 	detect=0
				# 	print("Registered User Detected =",det_peeps)		
				# 	# count=count+1
				# 	detect=0
				# 	break
				# else:
				# 	cv2.rectangle(frame,(rect[0],rect[1]),(rect[0] + rect[2],rect[1]+rect[3]),(255,0,0), 4)
				# 	cv2.putText(frame,"Unknown",(rect[0],rect[1]),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),1,cv2.LINE_AA)
...
def findPeople(features_arr, positions, thres = 0.99, percent_thres = 99):
	f = open('./utils/main_dataset.txt','r')
	data_set = json.loads(f.read());
	returnRes = [];
